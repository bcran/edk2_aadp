...
STATIC struct
{
  CHAR16 *Name;
  CHAR16 *FName;
  EFI_STATUS (*Func) (UINTN *Base, UINT32 *Size, UINTN *MiscBase, UINT32 *MiscSize);
}
BaseInfo_new[] =
{
...
STATIC EFI_STATUS Flash_GetInfo (CHAR16 *Str, UINTN *Base, UINT32 *Size, int *Index)
{
  ...
  for (UINTN i = 0; (Name = BaseInfo_new[i].Name) != NULL; i++) {

    if (StrniCmp (Str, Name, StrLen (Name)) == 0) {
      *Index = i;
      EFI_STATUS Status = BaseInfo_new[i].Func (Base, Size, NULL, NULL);
      if (EFI_ERROR (Status)) *Base = -1; // call error
      return Status;
    }
  }
